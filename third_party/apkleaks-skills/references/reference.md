# apkleaks — Deep Reference

> Loaded on demand. The lean [`SKILL.md`](../SKILL.md) covers the 90% path; come here for the
> full surface: every subcommand flag, the complete MCP lifecycle, the resource/prompt
> catalog, and the full error-code list.
...
When consumed as an installed plugin, the bundled MCP server launches automatically from the
plugin's install dir (`${CLAUDE_PLUGIN_ROOT}`). To wire it manually for local development, drop
one of these into `.claude/settings.json`; Claude Code starts the server when the project opens.
