...
(function(){
  ...
  function render(row,data){var box=row.querySelector('[data-progress]');if(!box)return;var stage=data.stage||'';var active=stage==='downloading'||stage==='queued'||stage==='fetching';if(!active){box.hidden=true;return;}box.hidden=false;var fill=box.querySelector('.dl-bar > i');var label=box.querySelector('.dl-label');var pct=Math.max(0,Math.min(100,data.percent||0));if(fill){if(stage==='queued'||stage==='fetching'||!(data.total>0)){fill.style.width=stage==='fetching'?'35%':'15%';}else{fill.style.width=pct+'%';}}if(label){if(stage==='queued'){label.textContent='Queued — waiting for download slot';return;}if(stage==='fetching'){label.textContent='Fetching Telegram message…';return;}var name=data.file||'file';var pos=(data.index&&data.count)?' ('+data.index+'/'+data.count+')':'';if(data.total>0){label.textContent=name+pos+' · '+pct+'% · '+human(data.done)+' / '+human(data.total);}else{label.textContent=name+pos+' · starting download…';}}}
  function pollOne(row){var id=row.getAttribute('data-job-id');return fetch('/jobs/'+encodeURIComponent(id)+'/progress',{credentials:'same-origin',headers:{Accept:'application/json'}}).then(function(r){if(!r.ok)throw new Error('http '+r.status);return r.json();}).then(function(data){render(row,data);var started=row.getAttribute('data-job-status');var stage=data.stage||'';return data.status&&data.status!==started&&stage!=='downloading'&&stage!=='queued'&&stage!=='fetching'?'changed':'ok';}).catch(function(){return'ok';});}
